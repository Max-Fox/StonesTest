...
struct ContentView: View {
    ...
    @State private var showPiramide = false
    @State private var needShowFade = false
    ...
    var body: some View {
        ZStack {
            NavigationView {
                ...
                .fullScreenCover(isPresented: $showPiramide) {
                    PiramidView(needShowFade: $needShowFade).clearModalBackground()
                }
            }
            
            if needShowFade {
                Color(UIColor(white: 0.3, alpha: 0.5)).ignoresSafeArea()
            }
